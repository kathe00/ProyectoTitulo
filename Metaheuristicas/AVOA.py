        ...
        for i in range(self.nsa):
            ...
            if abs(f) >= 1:
                ...
                if p1 >= self.rp1:
                    ...
                    for j in range(self.dim):
                        rand3 = random.random()
                        poblacion[i][j] = bbuitre[j] - f + (rand2 * ((self.ub - self.lb) * rand3 + self.lb))
                ...
                if abs(f) >= 0.5:
                    ...
                    if p3 >= self.rp3:
                        # Actualizar posición según eq. 3.13
                        for j in range(self.dim):
                            a1 = best1[j] - ((best1[j] * poblacion[i][j]) / (best1[j] - pow(poblacion[i][j],2))) * f
                            a2 = best2[j] - ((best2[j] * poblacion[i][j]) / (best2[j] - pow(poblacion[i][j],2))) * f
